...
这个题目要实现一个正则表达式引擎，比较难，但还好内容比较少，只需要处理普通字符和'.'以及'*'就可以了。正则表达式需要用到NFA和DFA，以及NFA朝DFA的转换，之前没有系统地学习过这方面的知识，所以下面的实现参考了 [[https://www.codeproject.com/Articles/5412/Writing-own-regular-expression-parser][这篇文章]] 和 [[http://charlee.li/parse-regex-with-dfa.html][这篇文章]] 。代码如下：
